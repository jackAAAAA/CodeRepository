...
//                    以下代码不能添加
//                    V2TIMMessage message1 = message.getV2TIMMessage();
//                    message1.getTextElem().getNextElem();
                }
            }

//            处理List<TUIMessageBean>
            @Override
            public void onRecvNewMessage(List<TUIMessageBean> messages) {
                if (groupInfo == null || !TextUtils.equals(messages.get(0).getGroupId(), groupInfo.getId())) {
                    TUIChatLog.i(TAG, "receive a new message , not belong to current chat.");
                } else {
                    GroupChatPresenter.this.onRecvNewMessage(messages);
                }
            }
