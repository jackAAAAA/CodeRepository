...
public class SuperVodListLoader {
    ...
    public ArrayList<VideoModel> loadDefaultVodList(Context applicationContext) {
        ...
        model.vipWatchModel = new VipWatchModel(applicationContext.getString(R.string.superplayer_vip_watch_tip), Long.MAX_VALUE);
//        试看时间为0时，表示试看结束
//        model.vipWatchModel = new VipWatchModel(applicationContext.getString(R.string.superplayer_vip_watch_tip), 0);
        list.add(model);

//        客户的点播视频加载【By appid & fileid & psign】
//        model = new VideoModel();
//        model.appid = 1252342885;
//        model.fileid = "387702295885412070";
//        model.title = "客户的点播视频"; //        有自定义备注，就显示自定义备注；否则自动生成点播视频名称
//        list.add(model);
...
//    }
    ...
    public void getVodInfoOneByOne(ArrayList<VideoModel> videoModels, OnVodInfoLoadListener listener) {
        if (videoModels == null || videoModels.size() == 0) {
            return;
        }
