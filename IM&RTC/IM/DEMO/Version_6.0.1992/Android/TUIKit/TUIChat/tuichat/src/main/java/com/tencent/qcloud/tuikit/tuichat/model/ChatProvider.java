...
public class ChatProvider {
    ...
    /**
     * *以下三个sendMessage的主要区别：
     *
     * *第一个sendMessage：通过ChatInfo设置消息推送实体（TPNS在线推送 & OEM离线推送）
     *
     * *第二个sendMessage VS 第一个sendMessage：区别在于前者采用了convertOfflinePushInfoToV2PushInfo来将OfflinePushInfo
     * *生成V2TIMOfflinePushInfo，以设置推送实体，
     * *并且可以通过设置onlineUserOnly来决定是否开启 阅后即焚；而后者根据聊天信息ChatInfo来手动生成V2TIMOfflinePushInfo，同时
     * *默认设置onlineUserOnly为false来关闭 阅后即焚。
     *
     * *第三个sendMessage VS 第二个sendMessage：区别在于前者默认设置onlineUserOnly为false来关闭 阅后即焚
     **/
