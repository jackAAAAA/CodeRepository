...
//            撤回消息的处理
            if (msg.isSelf()) {
                ...
                if (msg.getStatus() != TUIMessageBean.MSG_STATUS_SEND_FAIL) {
                    ...
                    action.setActionClickListener(new ChatPopMenu.ChatPopMenuAction.OnClickListener() {
                        @Override
                        public void onClick() {
//                          1.这里的mOnPopActionClickListener其实是
//                        ChatView#getMessageLayout().setPopActionClickListener(new MessageRecyclerView.OnPopActionClickListener())小括号中
//                                的对象
//                          2.此处调用的onReVokeMessageClick其实是ChatView中重写onRevokeMessageClick(TUIMessageBean msg)，而不是IDE引导的抽象方法
//
//                            3.action.setActionClickListener就是弹出界面点击的接口监听
//
//                            4.基于1 & 2 & 3就将 弹出界面点击的接口监听 与 ChatView弹出菜单的按钮监听 的绑定
                            mOnPopActionClickListener.onRevokeMessageClick(msg);
                        }
                    });
                    actions.add(action);
                }
