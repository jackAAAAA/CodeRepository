...
public class LocalVideoShareActivity extends TRTCBaseActivity implements View.OnClickListener {
    ...
    private MediaFileSyncReader.AudioFrameReadListener mAudioFrameReadListener = new MediaFileSyncReader.AudioFrameReadListener() {
        ...
        public void onFrameAvailable(byte[] data, int sampleRate, int channel, long timestamp) {
            ...
            mCustomFrameRender.onCapturedRawAudioFrame(trtcAudioFrame);
            mCustomFrameRender.onLocalProcessedAudioFrame(trtcAudioFrame);
            mCustomFrameRender.onRemoteUserAudioFrame(trtcAudioFrame, "test1028");
            mCustomFrameRender.onMixedPlayAudioFrame(trtcAudioFrame);
            mCustomFrameRender.onMixedAllAudioFrame(trtcAudioFrame);
            Log.d("onSDKVersion",TXLiveBase.getSDKVersionStr());
        }
    };
