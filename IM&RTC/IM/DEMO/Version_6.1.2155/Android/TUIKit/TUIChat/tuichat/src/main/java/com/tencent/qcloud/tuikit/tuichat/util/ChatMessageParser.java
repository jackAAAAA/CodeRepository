...
//       }
        ...
        /**
         * 此处使用了switch/case的组合 + do while{}:
         * 虽然能够实现组合消息子元Elem的提取，但是没有将资源Elem传递给onProcess函数导致无法正常解析子元组合元素
         *
         */
        int msgType = v2TIMMessage.getElemType();
        ToastUtil.toastLongMessage("解析的消息类型是：" + msgType);
        Log.e("解析的消息类型是：", "" + msgType);
...
//        Java反射 出现类无法解决发现的问题
//        尝试反射获取Message -> List< MessageBaseElement > getMessageBaseElements -> 在TUIMessageBean处理不同组元的消息
//        try {
...
//            Log.e(TAG, "反射结果为：" + (String) getMessageMethod.invoke(instance));
...
//        }
        ...
        do {
            if (elem instanceof V2TIMTextElem) {
                message = new TextMessageBean();
//                Android终端是以msgid来区分不同的message的
