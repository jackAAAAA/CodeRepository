package com.learning.javacodeexercise.ME1101;
...
public class GuseNumber {
    public static void main(String[] args) {
        ...
        while (!stopGame) {
            ...
            System.out.println("目标数字已经生成，数字在" + rangeStart + "到" + rangeEnd + "之间，不包括这连个数字。共有"
                    + guessTotal + "次猜测的机会。输入-1随时结束游戏。");
            ...
            while (guessLeft > 0) {
                ...
                if (guessNum <= rangeStart || guessNum >= rangeEnd) {
                    System.out.println("请输入在" + rangeStart + "到" + rangeEnd + "之间的数字，不包括这两个数字。");
                    continue;
                }
                //只要猜过一次，就算玩过了。
                if (gameStart) {
